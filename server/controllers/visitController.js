...
const checkIn = async (req, res) => {
  try {
    ...
    // 1. Validate input
    if (!visit_id || !latitude || !longitude) {
    ...
    const caregiverCoords = { latitude: lat, longitude: lng }
    const clientCoords = {
    ...
    await pool.query(
      ...
      [visit_id, caregiverId, lat, lng, accuracy || null, device_info || null]
    )
    ...
    if (isDuplicate) {
      await pool.query(
        `INSERT INTO alerts (visit_id, alert_type, severity, description)
         VALUES ($1, $2, $3, $4)`,
        [visit_id, 'suspicious_pattern', 'high', 'Multiple check-in attempts detected within 10 minutes']
      )
    }
  ...
  try {
    ...
    const updatedVisit = await pool.query(
      `UPDATE visits SET
       ...
       WHERE visit_id = $5
       RETURNING *`,
      [evvStatus, lat, lng, notes || null, visit_id]
    )
...
module.exports = { checkIn, checkOut, getVisitById, getAllVisits }